...
async def test_generate_answer_returns_source_metadata(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = await GeminiService(active_model=mock_model).generate_answer(
...
async def test_input_safety_classifier_blocks_adversarial_prompt(
    ...
    is_safe, reason = await GeminiService(active_model=mock_model).assess_input_safety(
...
async def test_input_safety_classifier_uses_cache(
    ...
    service = GeminiService(active_model=mock_model)
    first = await service.assess_input_safety("What ID can I use to vote?")
    second = await service.assess_input_safety("What ID can I use to vote?")
